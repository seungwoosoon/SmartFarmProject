...
function MyPage() {
  ...
  useEffect(() => {
    ...
    getProfileImageUrl()
        .then((url) => setProfileImageUrl(url || null)) // ✅ 변경: URL 또는 null 처리
  ...
  return (
      <div className="mypage-container">
        ...
        <p className="mypage-greeting">
          {t('mypage.greeting1')} <strong>{user?.name ?? t('mypage.defaultName')}</strong> {t('mypage.greeting2')} 🧑‍🌾
          <br />
          {t('mypage.greeting3')}
        </p>
        ...
        <div className="mypage-body">
          ...
          <div className="info-section">
            <p>• {t('mypage.nameLabel')}: {user?.name ?? '-'}</p>
            <p>• {t('mypage.numberLabel')}: {user?.phoneNumber ?? '-'}</p>
            <p>
              • {t('mypage.locationLabel')}: {user?.address?.street ?? ''}, {user?.address?.city ?? ''}, {user?.address?.zipcode ?? ''}
            </p>
            ...
            <button className="delete-btn">{t('mypage.deleteAccount')}</button>
          </div>
        </div>
        ...
        <p className="footer-info">
          SmartFarm {t('mypage.systemVersion')} v1.0.0 &nbsp;|&nbsp; © 2025 FarmLink Team
        </p>
      </div>
  );
}
