...
void ensureMqtt() {
  while (!client.connected()) {
    if (client.connect("SmartFarmBoard")) {
      ...
      // ※ led/status 같은 상태 브로드캐스트 없음 (pwm/mode 안 보냄)
    } else {
      delay(2000);
    }
  }
...
void loop() {
  ...
  if (millis() - t_pub > 5000) {
    ...
    humi = soil; // ★ CHANGED: '습도' 필드에 토양수분(%)을 사용

    StaticJsonDocument<256> doc;
    doc["temp"] = temp;
    doc["humi"] = humi;   // ★ CHANGED: 토양수분 값이 들어감(단위 %)
    doc["soil"] = soil;   // 필요하면 유지(대시보드 호환용)
    doc["cds"]  = cds;    // RAW (0~4095)
    doc["tds"]  = tds;
    doc["ph"]   = ph;
    ...
    // 시리얼: H에 토양수분이 표시됨
    Serial.printf("PUB -> T:%.1f H:%.0f S:%.0f%% L:%.0f TDS:%.1f pH:%.1f\n",
                  temp, humi, soil, cds, tds, ph);
  }
}
